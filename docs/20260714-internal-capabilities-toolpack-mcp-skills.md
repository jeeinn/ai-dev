...
- [ ] coder 指定 `coder-default` 与今日 DefaultTools 行为一致  
- [ ] analyze 无法调用写工具（registry 层拒绝）

**Analyze 短 Loop**

- [ ] Assign analyze → 评论中出现 **真实仓库路径/符号**（非纯臆测）  
- [ ] 工作区无新分支推送；无 PR  
- [ ] clone 失败时行为符合配置（失败注释或弱降级）

**Skills**

- [ ] 能发现 Gateway 目录与仓库内 `SKILL.md` 的 name/description  
- [ ] 激活后正文进入上下文；未激活不占满 token  

**MCP**

- [ ] 远程 server 工具出现在 Loop 可调列表；鉴权失败可读  
