...
| 表 | 用途 | 关键字段 |
|---|---|---|
| `agents` | AI Agent 配置 | name, gitea_username, provider, model, max_tokens, temperature, system_prompt, user_template, loop_config(JSON), sandbox_config(JSON), status |
...
两个认证层：
- **Bearer Token** (`h.auth.Wrap`): 简单 API token，用于 Agent、Route、Task、Prompt 管理接口（`api.auth_token` 配置）
...
| GET | `/api/agents/{id}/routes` | Token | Agent 的路由规则 |
...
| GET | `/api/routes` | Token | 路由规则列表 |
| POST | `/api/routes` | Token | 创建路由规则 |
| DELETE | `/api/routes/{id}` | Token | 删除路由规则 |
...
页面：
- **Dashboard** — 状态概览、统计数据、新用户引导、最近任务/Agent 快捷跳转
- **Agents** — Agent 列表（客户端分页）、创建/编辑（模板选择、配置分组折叠）
- **AgentDetail** — Agent 详情（基本信息、触发规则管理、Prompt 版本历史）
...
- Agent 详情页的触发规则表会显示"预计执行行为"，让用户心里有底
...
│   ├── Agents.vue                    #   Agent 列表 + 创建/编辑
│   ├── AgentDetail.vue               #   Agent 详情 + 触发规则 + Prompt 版本
